...
void Font::Draw(Player &player,std::string text, float x, float y, float scale, glm::vec3 color)
{
		// activate corresponding render state	
		m_Shader.Set3Float("textColor", color.x, color.y, color.z);
		m_Shader.SetMat4("projection", player.Projection);
		...
		for (c = text.begin(); c != text.end(); c++)
		{
			...
			glBindBuffer(GL_ARRAY_BUFFER, m_VBO.m_VertexBuffer);
...
void Font::BindShader()
{
	m_Shader.Bind();
}

void Font::UnBindShader()
{
	m_Shader.UnBind();
}
